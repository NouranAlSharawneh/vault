...
describe("vault://asset", () => {
  it("serves a file inside the vault with its media type", () => {
    const where = ask("vault://asset/atlas-api/assets/diagram.png", [
  ...
  it("decodes the path, so a space in a filename still resolves", () => {
    const where = ask("vault://asset/inbox/assets/my%20shot.png", [
  ...
  it("refuses a path that climbs out of the vault", () => {
    ...
    expect(ask("vault://asset/%2e%2e%2f%2e%2e%2fetc%2fpasswd.png")).toEqual({ deny: 403 });
    expect(ask("vault://asset/inbox/..%2f..%2fsecrets.png")).toEqual({ deny: 403 });
  });
  ...
  it("never sees plain `../` — the URL parser resolves it into the vault first", () => {
    ...
    expect(ask("vault://asset/../../etc/passwd.png")).toEqual({ deny: 404 });
  });
  ...
  it("serves nothing when no vault is open", () => {
    expect(
      resolveAssetRequest(undefined, new URL("vault://asset/a.png"), () => true, insideVault),
    ).toEqual({ deny: 404 });
  });
