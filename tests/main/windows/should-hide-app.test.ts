...
describe("shouldHideApp", () => {
  it("Esc / ⌘↵ return to the app the sheet was summoned over, even with Vault open", () => {
    expect(hide("dismiss", true, true)).toBe(true);
    expect(hide("dismiss", true, false)).toBe(true);
  });

  it("Esc / ⌘↵ leave focus in Vault when Vault was already in front", () => {
    expect(hide("dismiss", false, true)).toBe(false);
  });

  it("with no Vault window left, the app hides whatever the reason, bar a handoff", () => {
    expect(hide("dismiss", false, false)).toBe(true);
    expect(hide("blur", false, false)).toBe(true);
    expect(hide("blur", true, false)).toBe(true);
  });

  it("a blur never hides a Vault window the user can still see", () => {
    expect(hide("blur", true, true)).toBe(false);
    expect(hide("blur", false, true)).toBe(false);
  });

  it("⌥⌘↵ and Open in editor never hide Vault — it is about to show something", () => {
